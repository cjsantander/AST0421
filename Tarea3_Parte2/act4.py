def query_TAP(tap_endpoint, adql_query, table_to_upload=None):
    ...
    r = requests.post(tap_endpoint + '/sync', data={
                      'query': adql_query, 'request': 'doQuery',
                      'lang': 'adql', 'format': 'votable', 'phase': 'run'})
...
def plot_scatter_density(data, xcol, ycol, xlabel,
                         ylabel, title, xlim=None, ylim=None):
    import matplotlib.pyplot as plt
    from scipy.stats import gaussian_kde
    import numpy as np

    x = np.reshape(np.array(data[xcol], copy=False).astype(
        'float'), (len(data['B-V'])))
    y = np.reshape(np.array(data[ycol], copy=False).astype(
        'float'), len(data['B-V']))
